...
const BUILDIN_PLUGINS_FOLDER = path.join(__dirname, '../plugins/');
const BUILDIN_PLUGINS = ['regExpMessage', 'shortReply'];

const USER_PLUGIN_FOLDER = path.join(__dirname, '../plugins/');
...
class TeleBot {
    ...
    constructor(cfg) {
        ...
        this.processUpdate = (update, props) => {
            console.log('handle Update:', update, props)
            if (update) {
    ...
    start() {
        ...
        if (this.webhook) {
            ...
            return this.setWebhook(url, cert, this.allowedUpdates, this.maxConnections).then(() => {
                ...
                if (this.webhook.host && this.webhook.port) {
            ...
            // Delete webhook
            this.deleteWebhook().then((response) => {
                f.poll = true;
